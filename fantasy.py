...
def colorScheme(team):
    colors = {'Lakers1':'#552583','Lakers2':'#FDB927','Clippers1':'#C8102E','Clippers2':'#1D428A','Hawks1':'#E03A3E','Hawks2':'#C1D32F','Celtics1':'#007A33','Celtics2':'#BA9653', 
          ...
          'Blazers1':'#E03A3E','Blazers2':'#000000','Kings1':'#5A2D81','Kings2':'#63727A','Spurs1':'#C4CED4','Spurs2':'#000000','Raptors1':'#CE1141','Raptors2':'#000000',
...
#create all possible combinations of the pool given captain and salary constraint

def lineupCombinations(captain, pool, dfs):
    #creates the pool without the captain
    x = np.where(pool == captain)
    pool = np.delete(pool, x[0])

    draft = np.empty([1,8])
    comb = combinations(pool, 5)

    for i in list(comb):
        #tuple to contain captain = utility players
        newi = (captain,) + i
        
        #find total salary of tuple and remove if it exceeds $50000
        tot_salary = 0
        cpt_salary = 1.5*float(dfs[dfs.index == captain]['Salary'])
        tot_salary += cpt_salary
        for k in i:
            util_salary = float(dfs[dfs.index == k]['Salary'])
            tot_salary += util_salary
        if tot_salary < 50000:
            sali = newi + (tot_salary,)

            #find total expected DFS points of the tuple
            tot_pts = 0
            cpt_pts = 1.5*float(dfs[dfs.index == captain]['DFS'])
            tot_pts += cpt_pts
            for j in i:
                util_pts = float(dfs[dfs.index == j]['DFS'])
                tot_pts += util_pts
    
            #tuple to contain draft and expected DFS points
            sali = sali + (tot_pts,)
    
            #append values to initialized array
            draft = np.vstack((draft,sali))
        
    df = pd.DataFrame(draft, columns = ['CPT','UTIL1','UTIL2','UTIL3','UTIL4','UTIL5','Salary','Expected DFS'])
    df['Salary'] = df['Salary'].astype(float)
    df['Expected DFS'] = df['Expected DFS'].astype(float)
    return df[1:]

def lineupCombinations1(captain, pool, dfs):
    #creates the pool without the captain
    ...
    for i in list(comb):
        ...
        if tot_salary < 50000:
            ...
            #find the standard deviation of the lineup operating under assumption that they are independent
            tot_var = 0
            cpt_var = (float(dfs[dfs.index == captain]['DFS STD']))**2
            tot_var += cpt_var
            for j in i:
                util_var = (float(dfs[dfs.index == j]['DFS STD']))**2
                tot_var += util_var
            dfs_std = round(tot_var**0.5, 3)
            
            #tuple to contain draft, expected points, and standard deviation
            sali = sali + (dfs_std,)
    ...
    df = pd.DataFrame(draft, columns = ['CPT','UTIL1','UTIL2','UTIL3','UTIL4','UTIL5','Salary','Expected DFS', 'STD'])
    df['Salary'] = df['Salary'].astype(float)
    df['Expected DFS'] = df['Expected DFS'].astype(float)
    return df[1:]

def lineupCombinations2(captain, pool, dfs):
    #creates the pool without the captain
    cpt = np.array(captain)
    arr = np.setdiff1d(pool, cpt)
    ...
    sea_fp = list(dfs['Season FP'])
    l10_fp = list(dfs['Last 10 FP'])
    
    sal_dict = dict(zip(plys, sals))
    sea_fp_dict = dict(zip(plys, sea_fp))
    l10_fp_dict = dict(zip(plys, l10_fp))
    ...
    for i in list(comb):
        ...
        if ((tot_salary < 50000) and (tot_salary > 40000)):
            ...
            cpt_2 = 1.5*l10_fp_dict[captain]
            tot_pts += cpt_pts
            tot_2 += cpt_2
            for j in i:
                util_pts = sea_fp_dict[j]
                util_2 = l10_fp_dict[j]
                tot_pts += util_pts
                tot_2 += util_2
    ...
    df = pd.DataFrame(draft, columns = ['CPT','UTIL1','UTIL2','UTIL3','UTIL4','UTIL5','Salary','Season FP', 'Last 10 FP'])
    df['Salary'] = df['Salary'].astype(float)
    df['Season FP'] = df['Season FP'].astype(float)
    df['Last 10 FP'] = df['Last 10 FP'].astype(float)
    return df[1:]
...
def generateLineups(pool, draft):
    column_names = ['CPT','UTIL1','UTIL2','UTIL3','UTIL4','UTIL5','Salary','Expected DFS', 'STD']
    df = pd.DataFrame(columns = column_names)
    for i in pool:
        captain = i
        df_temp = lineupCombinations1(captain, pool, draft)
        df = df.append(df_temp)
    return df

def generateLineups1(pool, draft):
    column_names = ['CPT','UTIL1','UTIL2','UTIL3','UTIL4','UTIL5','Salary','Season FP', 'Last 10 FP']
    df = pd.DataFrame(columns = column_names)
    for i in pool:
        captain = i
        df_temp = lineupCombinations2(captain, pool, draft)
        df = df.append(df_temp)
    return df
...
#this function is the first step in creating lineups it pulls from files given a home team, away team, and the day
def draftMerge(away_team, home_team, date):
    dfh = matchupSummary(home_team, away_team) #getting the historic data of the home team
    dfa = matchupSummary(away_team, home_team) #getting the historic data of the away tean
    
    dfh = pd.concat([dfh, dfa])
    gm_players = dfh.index.unique()
    ...
    draft.drop(columns = {'Game Info', 'Name + ID', 'ID', 'AvgPointsPerGame'}, inplace = True)
    draft = draft[draft['Roster Position'] != 'CPT']
    draft = draft.rename(index = {'Robert Williams':'Robert Williams III'})
    
    #append values to draft doc
    season_perf = []
    season_l10 = []
    for i in draft.index:
        try:
            p_season_perf = dfh[dfh.index == i]['Season'][0]
        except:
            p_season_perf = 'NaN'
        try:
            p_season_l10 = dfh[dfh.index == i]['Last 10'][0]
        except:
            p_season_l10 = 'NaN'            
        season_perf.append(p_season_perf)
        season_l10.append(p_season_l10)
    draft['Season FP'] = season_perf
    draft['Last 10 FP'] = season_l10

    return draft, gm_players
...
#at least 2 core players from each team
...
def filterCorePlayers_noteam(core, df):
    ...
    #df_filtered = pd.DataFrame(data = frames, columns = column_names)
    df_filtered = df_filtered.drop_duplicates()
...
def valueProbability(df_filtered, sim_games):
    #creating a list of dictionaries of the simulated games
    sim_np = sim_games.to_numpy()
    #players in order of sim_games
    plys = list(sim_games.columns)
    ...
    #util matrix
    utl_df = df_filtered.drop(['CPT', 'Salary', 'Season FP', 'Last 10 FP'], axis = 1)
...
def minPlayed(df, min_dict_5, min_dict_10):
    df_np = df.drop(['Salary','Season FP','Last 10 FP','Sims Mean','Sims STD','Probs 4x','Probs 5x'], axis = 1).to_numpy()
    min_10 = []
    min_5 = []
    for i in df_np:
        min_row_10 = 0
...
def usageMetric(df, players):
    ...
    df_np = df.drop(['Salary', 'Season FP', 'Last 10 FP', 'Sims Mean', 'Sims STD', 'Probs 4x', 'Probs 5x', 'L10 Min', 'L5 Min'],axis = 1).to_numpy()
    ...
    #print(l5_metric_dict)
    #print(l10_metric_dict)
...
def FAQ(pdf, site):
    ...
    pdf.ln(3)
    pdf.set_font('DejaVu', 'I', 9)
    pdf.cell(18, h = 5, txt = "With your eyes.", border = 0, ln = 1, align = 'L')
    ...
    pdf.ln(3)
    pdf.set_font('DejaVu', 'I', 9)
    pdf.cell(18, h = 5, txt = "Magic... with a sprinkle of python.", border = 0, ln = 1, align = 'L')
